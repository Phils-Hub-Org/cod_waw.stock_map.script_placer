...
class ScriptPlacer:
    ...
    def copyFiles(self, waw_root, modName, mapName, mode):
        ...
        # so if dev, e.g. vscode, then we'll set the base path as the current working directory
        # if not, then we'll use the temp unpack path
        if isExecutable():
            template_files_root = getTempUnpackPath()
        else:
            template_files_root = os.getcwd()
        
        logger.debug(f'template_files_root: {template_files_root}')
        
        template_files_dir = os.path.join(template_files_root, "stock_base_files", mapName)
        ...
        destination_dir = os.path.join(waw_root, "mods", modName)
